...
export function createRosterScreen(store: GameStore): HTMLElement {
    ...
    if (hasPlayers) {
        ...
        const useBtn = el<HTMLButtonElement>(
            'button',
            'btn btn--primary btn--xl btn--full'
        );
        useBtn.textContent = '✓  Usar estes jogadores';
        useBtn.addEventListener('click', () =>
            store.update(Actions.useRoster(savedPlayers))
        );
        ...
        const continueBtn = el<HTMLButtonElement>(
            'button',
            'btn btn--primary btn--xl btn--full'
        );
...
function buildRosterChip(
    ...
    avatar.style.background =
        AVATAR_COLORS[idx % AVATAR_COLORS.length] ?? '#7C3AED';
