...
function buildSelectingScreen(store: GameStore): HTMLElement {
  ...
  footer.style.cssText =
    'padding-top:.75rem;padding-bottom:max(1.5rem, env(safe-area-inset-bottom));';
...
function buildShowingScreen(store: GameStore): HTMLElement {
  ...
  // ── Turn banner (compact) ─────────────────────────────────
  const banner = buildCompactBanner(currentPlayer, currentPlayerIndex, players.length);
  ...
  footer.style.cssText =
    'padding-top:1rem;padding-bottom:max(1.5rem, env(safe-area-inset-bottom));';
...
function buildGameCard(card: Card, activePlayer: Player, targetPlayer?: Player): HTMLElement {
...
function buildPenaltyOverlayInScreen(
  ...
  doneBtn.addEventListener('click', () => store.update(Actions.dismissPenalty()));
...
function buildCompactBanner(player: Player, idx: number, total: number): HTMLElement {
