...
const GlassDistortion = {
  ...
  _innerDist(
    ...
    const outer = Math.hypot(Math.max(qx, 0), Math.max(qy, 0)) + Math.min(Math.max(qx, qy), 0) - r;
  ...
  build(
    ...
    for (let i = 0; i < W * H; i++) {
      ...
      if (mag > 0) {
        ...
        const dx = dxArr[i] ?? 0;
        const dy = dyArr[i] ?? 0;
        d[idx] = Math.min(255, Math.max(0, Math.trunc(128 + dx * n * 127 + 0.5)));
        d[idx + 1] = Math.min(255, Math.max(0, Math.trunc(128 + dy * n * 127 + 0.5)));
      } else {
        d[idx] = 128;
        d[idx + 1] = 128;
      }
  ...
  initDock(): void {
    ...
    if (rect.width > 4 && rect.height > 4) this._applyToDock(rect.width, rect.height);
  },
